...
-}
----basicM :: Mapa -> Bool
----basicM m = m == head mapas16 ||elem m mapas128 || elem m mapas64 
{-|
...
-- extract_p kl l t | length t >= length l = (Este, True)

extract_p kl l t | length t == 1 = cmp (head t,head t) (length t - 1) (kl,l) --cmp (l !!( length t - 1 ) )
                 | otherwise     = cmp (t!!1 ,head t) (length t - 1) (kl,l) --cmp (l !!( length t - 1 ) )

      where cmp :: (Posicao,Posicao) -> Int -> (Mapa,[ (Posicao ,Orientacao)])-> (Orientacao,Bool)
            cmp _ (-1) _                = (Este, True)
            cmp (prevp,p) n (kl,l) | n>0  && fst (l!!n) == p && fst (l!!(n-1)) == prevp  = ( snd (l!!n), (isCurv kl (fst (l!!(n-1)) )) && (isCurv kl (fst (l!! (n) )) ) && (isCurv kl (fst (l!! (n+1) )) ) )
                                   | n==0 && fst (l!!n) == p                             = ( snd (l!!n) , True)
                                   | otherwise                                           = cmp (prevp,p) (n - 1) (kl,l)
...
caution x ct | x > 0.2 && x <= 0.7  = ct { acelerar = False ,  nitro = Nothing }
             | x <= 0.2   = ct { acelerar = False , travar = True , nitro = Nothing }
             | x > 0.7 && x < 1   = ct { nitro = Nothing }
             | otherwise = ct
{-|
