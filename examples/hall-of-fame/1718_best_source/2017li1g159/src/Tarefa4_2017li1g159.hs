...
-}
module Tarefa4_2017li1g159 where
...
give_Nitro dt (Just id) nt prop lcc w = replaceAtIndex id ( ( lcc !! id ) { velocidade = aux (min dt nt) ( lcc !! id ) w (k_nitro prop ) } ) lcc
            ...
            where aux :: Tempo -> Carro-> Angulo -> Double -> Velocidade 
                  aux t (Carro _ wi v ) w k_n = soma  (scale (t*) (scale (k_n*) (get_vec  w ))) v 
...
updateHist state id =  state { historico = replaceAtIndex id (f (carros state !! id) (historico state!! id)) (historico state) }
                where f :: Carro -> [Posicao] -> [Posicao]
                      f  c []   = let (u,v) = posicao c  in  [scale floor (u,v)] 
                      f  c path = let (u,v) = posicao c  in check path (scale floor (u,v)) 
                                    where check :: [Posicao] -> Posicao -> [Posicao]
                                          check (h:t) r     | h == r    = h:t
                                                            | otherwise = r:h:t
...
new_velocity action prop cc t or prevA | or == Nothing = soma (scale (t*) (soma (soma a b) c ) ) (velocidade cc)
                                       | otherwise     = soma (scale (t*) (soma (soma (soma a b) c ) d) ) (velocidade cc)
...
acAtrito k_at (x,y) = scale ((-k_at ) *) (x,y) 
                     --   where unit = scale ((1/norm)*) (x,y) 
                       --       norm = sqrt (x*x + y*y)
{-|
...
acgravitica k_p o = scale ((- k_p) *) dram
...
acAcel action k_ac w | travar action == acelerar action = (0,0)
                     | acelerar action = let (a,b) = get_vec w in scale (k_ac *) (a,b)
                     | otherwise       = let (a,b) = get_vec w in scale ((-k_ac) *) (a,b)
